...
public abstract class AbstractSAMLCallbackHandler implements CallbackHandler {
   ...
   private static final Logger logger = LoggerFactory.getLogger(AbstractSAMLCallbackHandler.class);
   ...
   protected AuthenticationStatementBean authBean;
   protected AuthDecisionStatementBean authzBean;
   ...
   protected void createAndSetStatement(SubjectBean subjectBean, SAMLCallback callback) {
      if (statement == Statement.AUTHN) {
         AuthenticationStatementBean authenBean = new AuthenticationStatementBean();
         if (this.authBean != null) { // set by user
            authenBean = this.authBean;
            ...
            if (subjectLocalityIpAddress != null || subjectLocalityDnsAddress != null) {
               ...
               authenBean.setSubjectLocality(subjectLocality);
            }
            authenBean.setAuthenticationInstant(new DateTime());
            authenBean.setAuthenticationMethod("PasswordProtectedTransport");
            System.out.println("Authentication Bean not null ");
         }

         callback.setAuthenticationStatementData(Collections.singletonList(authenBean));
         ...
         AuthDecisionStatementBean authorizationBean = new AuthDecisionStatementBean();
         if (this.authzBean != null) { // set by user
            authorizationBean = this.authzBean;
         } else { // default value
            if (subjectBean != null) {
               authorizationBean.setSubject(subjectBean);
            }
            ActionBean actionBean = new ActionBean();
            actionBean.setContents("Read");
            authorizationBean.setActions(Collections.singletonList(actionBean));
            authorizationBean.setResource("endpoint");
            authorizationBean.setDecision(AuthDecisionStatementBean.Decision.PERMIT);
            authorizationBean.setResource(resource);
         }
   ...
   protected KeyInfoBean createKeyInfo() throws Exception {
      ...
      if (statement == Statement.AUTHN) {
         logger.debug("Certificate use to create key info : " + certs[0].getIssuerDN());
