 ...
 *
 * -------------------------------------------------------------------------
 * $Id: XMLUtils.java 98 2006-02-24 16:18:48Z alouis $
 * -------------------------------------------------------------------------
 */
...
import org.wsssoapbox.soap.exception.InputValidationException;
...
public final class XMLUtils {
    ...
    public static String parseToString(final Node node) throws TransformerException {
        ...
        if (node != null) {
            ...
            XMLUtils.defaultTransformerThreadLocal.get().transform(new DOMSource(node), new StreamResult(stringWriter));
    ...
    public static List getElementsByTagNameNS(Element element, String nsName,
            String tagName) {
    ...
    public static Element getFirstChildByTagName(Element parent, String name) {
        List<Element> list = XMLUtils.getChildrenByTagName(parent,
                new String[]{name});
        if (list.size() > 0) {
            return list.get(0);
        } else {
    ...
    public static String createStringFromDOMNode(final Node node, final boolean omitDeclaration)
        ...
        if (omitDeclaration) {
            XMLUtils.transformerWithoutXmlDeclarationThreadLocal.get().transform(source, resultStream);
        } else {
            XMLUtils.defaultTransformerThreadLocal.get().transform(source, resultStream);
        }
        return out.toString();
    }
    ...
    public static Document createDocumentFromString(final String xml) {
        ...
        try {
            ...
            doc = XMLUtils.documentBuilderThreadLocal.get().parse(inputSource);
            doc.normalize();
    ...
    public static QName extractXmlAttributeQName(final Node node, final String attrName) {
        String attr = XMLUtils.getAttributeValue(node, attrName);
    ...
    public static QName extractRequiredXmlAttributeQName(final Node node, final String attrName) {
        String attr = XMLUtils.getRequiredAttributeValue(node, attrName);
    ...
    public static void writeDocument(final Document document, final OutputStream outputStream) throws Exception {
        ...
        if (document != null && outputStream != null) {
            XMLUtils.defaultTransformerThreadLocal.get().transform(new DOMSource(document),
    ...
    public static void printDocument(final Document document, final OutputStream outputStream) throws Exception {
    ...
    public static Document loadDocument(InputStream inputStream) throws IOException, SAXException {
        ...
        try {
            document = XMLUtils.documentBuilderThreadLocal.get().parse(inputStream);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return document;
    ...
    public static String prettyPrintDoc(Document message) {
        try {
    ...
    public static String prettyPrint(SOAPMessage message) throws SOAPException {
        String unformattedString = "";
        try {

            ByteArrayOutputStream out = new ByteArrayOutputStream();
            message.writeTo(out);
            unformattedString = new String(out.toByteArray());
    ...
    private String getInputString(DataHandler dh) throws InputValidationException {
        ...
        if (dh == null) {
            ...
            try {
                responseBytes = XMLUtils.getBytesFromDataHandler(dh);
                System.out.println("responseBytes length : " + responseBytes.length);
            } catch (IOException e) {
                failMsg =
    ...
    public static String toStringNode(Node n)
            throws Exception {
    ...
    public static void toStringDocument(Document document, String outputFile)
